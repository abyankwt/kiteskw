...
async function getOverview() {
    const [enrollments, revenue, courses, users, todayEnrollments, monthlyEnrollments, monthlyRevenue, views] = await Promise.all([
        pool_1.default.query(`SELECT COUNT(*) FROM enrollments WHERE status = 'active'`),
        pool_1.default.query(`SELECT COALESCE(SUM(amount_paid), 0) as total FROM enrollments WHERE status = 'active'`),
        pool_1.default.query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = 'published') as published FROM courses`),
        pool_1.default.query(`SELECT COUNT(*) FROM users WHERE is_active = true`),
        pool_1.default.query(`SELECT COUNT(*) FROM enrollments WHERE status = 'active' AND enrolled_at >= CURRENT_DATE`),
        pool_1.default.query(`SELECT COUNT(*) FROM enrollments WHERE status = 'active' AND enrolled_at >= DATE_TRUNC('month', CURRENT_DATE)`),
        pool_1.default.query(`SELECT COALESCE(SUM(amount_paid), 0) as total FROM enrollments WHERE status = 'active' AND enrolled_at >= DATE_TRUNC('month', CURRENT_DATE)`),
        pool_1.default.query(`SELECT COUNT(*) FROM course_views WHERE viewed_at >= CURRENT_DATE`),
    ]);
...
async function getEnrollmentsOverTime(from, to, groupBy = 'day') {
    ...
    const { rows } = await pool_1.default.query(`SELECT
     ...
     ORDER BY date ASC`, [trunc, from, to]);
    return rows.map(r => ({
        date: r.date,
